...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	if( $f_master_bug_id > 0 ) {
		...
		//comentando os campos não usados PROSEGUR 27/02/2011
//		$f_build				= $t_bug->build;
...
<br /><center>
<? 
//exibe o submenu com as opções de gerar uma tarefa integrada ou gerar normal PROSEGUR 15/09/2011
print_bracket_link( 'task_report_integrated_page.php', lang_get( 'integrated_method' ) ); 
print_bracket_link( '', lang_get( 'normal_method' ) ); 
?>
</center>
<div align="center">
...
<tr>
	<td class="form-title" colspan="2">
		...
		<?php
		//esse conteúdo estava fora do td do title e dentro de outro td sozinho, não vi essa necessidade e estava quebrando o layout por isso foi removido o outro td PROSEGUR 15/09/2011
		//esse código é para exibir a opção de relatório simples do mantis, porém no nosso caso o padrão é ser o avançado somente
...
<? /* comentado para remover o campo do relatório PROSEGUR 22/02/2011
<!-- Category -->
...
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
		<?php echo lang_get( 'custom_field_type' ) ?> <?php print_documentation_link( 'custom_field_type' ) ?>
	</td>
	<td>
		<select <?php echo helper_get_tab_index() ?> name="type">
			<?php print_enum_string_option_list( 'type' ) ?>
		</select>
		<?php /*campo criado para digitar os casos associados na mão, removido porque não foi usado, talvez no futuro
		echo lang_get( 'numero' ) ?>
		<input onkeyup='javascript:txtfield()' onKeyPress="javascript:return soNumeroVirg()" <?php echo helper_get_tab_index() ?> type="text" name="numero" size="4" maxlength="100" value="" id="txtnum" />
		<font size='1'><?php echo lang_get( 'multiplos_casos' ) ?></font>
		*/ ?>
	</td>
</tr>
<?php } ?>

<? /* comentado para remover o campo do relatório PROSEGUR 16/02/2011
...
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
		<span class="required">*</span><?php echo lang_get( 'start_date' ) ?>
	</td>
	<td>
		<input <?php echo helper_get_tab_index() ?> type="text" name="data_inicio" id="data_inicio" size="8" maxlength="10" value="" /> <img src="images/calendar-img.gif" onclick="displayCalendar(document.getElementById('data_inicio'),'dd/mm/yyyy',this)">
		<span class="required">*</span><?php echo lang_get( 'end_date' ) ?>
		<input <?php echo helper_get_tab_index() ?> type="text" name="data_fim" id="data_fim" size="8" maxlength="10" value="" /> <img src="images/calendar-img.gif" onclick="displayCalendar(document.getElementById('data_fim'),'dd/mm/yyyy',this)">
...
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
		<?php echo lang_get( 'hora_previsao' ) ?> <?php print_documentation_link( 'hora_previsao' ) ?>
	</td>
	<td>
		<input <?php echo helper_get_tab_index() ?> onKeyPress="javascript:return soNumero()"; type="text" name="hours_prev" id="hours_prev" size="1" maxlength="3"/>
		<?php echo lang_get( 'horas'); ?> 
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
		<span class="required">*</span><?php echo lang_get( 'headline' ) ?> <?php print_documentation_link( 'headline' ) ?>
	</td>
	<td>
		<input <?php echo helper_get_tab_index() ?> type="text" name="summary" id="summary" size="105" maxlength="120" value="<?php echo $f_summary ?>" />
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		<textarea <?php echo helper_get_tab_index() ?> name="description" id="description" cols="80" rows="10"><?php echo $f_description ?></textarea>
	</td>
</tr>

<? /* retirado o campo PROSEGUR 16/02/2011
<!-- Steps to Reproduce -->
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
...
<tr>
	...
	<td class="center">
		<input <?php echo helper_get_tab_index() ?> type="submit" class="button" onclick='javascript:return validaCampos()' value="<?php echo lang_get( 'submit_report_button' ) ?>" />
	</td>
</tr>
...
<!--
	window.document.report_bug_form.type.focus();
-->
</script>
<?php } ?>
