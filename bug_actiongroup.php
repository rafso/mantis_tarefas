...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	#recebe o texto da anotação e se ela é privada ou pública PROSEGUR
		...
		case 'UP_STATUS':
			...
			if ( access_has_bug_level( access_get_status_threshold( $f_status, $t_project ), $t_bug_id ) ) {
				if ( TRUE == bug_check_workflow($t_status, $f_status ) ) {
					...
					bugnote_add( $t_bug_id, $f_bugnote_text, '0:00', $f_private );
					helper_call_custom_function( 'issue_update_notify', array( $t_bug_id ) );
					//inserido para enviar e-mail mesmo que seja utilizado essa opção, antes não enviava PROSEGUR 06/07/2009
					//chama a função send_mail_status que envia um e-mail de acordo com status que será alterado
					send_mail_status($t_bug_id, $f_status);  //chama a função passadno o bug e o novo status
				} else {
					$t_failed_ids[$t_bug_id] = lang_get( 'bug_actiongroup_status' );
				}
			} else {
