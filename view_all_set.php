...
<?php
	...
	if ( is_array( gpc_get( 'show_status', null ) ) ) {
		...
		$f_show_type = array();
	if ( is_array( gpc_get( 'show_type', null ) ) ) {
		$f_show_type = gpc_get_string_array( 'show_type', META_FILTER_ANY );
	} else {
		$f_show_type = gpc_get_string( 'show_type', META_FILTER_ANY );
		$f_show_type = array( $f_show_type );
	}
...
/*   array contents
	 ...
	 2: $f_show_severity
	 3: $f_show_status
	 3: $f_show_type	//adicionar PROSEGUR 01/06/2011
	 4: $f_per_page
...
*/
	...
	switch ( $f_type ) {
		...
		case '1':
				...
				$t_setting_arr['show_type'] = $f_show_type; //adicionado PROSEGUR 01/06/2011
