...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	function print_avatar( $p_user_id, $br = null ) { //adicionado o parametro não obrigatório br para saber se imprime ou não com <br / > na tela
		if ( !user_exists( $p_user_id ) ) {
			return;
		}

		//foi alterado para funcionar com a imagem local, pega o username e procura username.png PROSEGUR 01/06/2011
		if ( access_has_project_level( config_get( 'show_avatar_threshold' ), null, $p_user_id ) ) {
			...
			if ($br = null){
				echo '<img class="avatar" id="avatarImg" src="' . $t_avatar_url . '" alt="' . user_get_field( $p_user_id, 'realname' ) .'" /></br>';
			} else {
				echo '<img class="avatar" id="avatarImg" src="' . $t_avatar_url . '" alt="' . user_get_field( $p_user_id, 'realname' ) .'" />';
			}
			}
		}
	}
	...
	//print genérico que pega um enum string qualquer e joga em um option PROSEGUR 09/06/2011
	function print_generic_option_list($t_enum_string){
		$type = explode_enum_string(config_get($t_enum_string));
		foreach ( $type as $t_type ) {
			$t_type = explode(":", $t_type);
			echo "<option value=\"$t_type[0]\"";
			echo ">$t_type[1]</option>";
		}	
	}
