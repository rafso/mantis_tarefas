...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	function filter_get_bug_rows( &$p_page_number, &$p_per_page, &$p_page_count, &$p_bug_count, $p_custom_filter = null, $p_project_id = null, $p_user_id = null, $p_show_sticky = null, &$p_id_array = null ) {
		...
		# filtro de data da tarefa, pela data de início PROSEGUR 13/06/2011
		...
		//filtro pelas tarefas que foram ou não enviadas no ultimo relatório com 100% - PROSEGUR 05/08/2010
	...
	//Faz a verificação de visibilidade, ela verifica se o usuário tem permissão de visualização, ou está atribuído a ele PROSEGUR 01/06/2011

	//verifica se a tarefa está atribuída à pessoa, se tiver mostra sem outras verificações
	if ( $row['handler_id'] == current_user_get_field('id') ) {
					$t_id_array[] = db_prepare_int ( $row['id'] );
	} else {
		//verifica se a tarefa é privada
		if ( VS_PRIVATE == $row['view_state'] ) {
			//se for verifica se o usuário tem acesso para ver as tarefas privadas
			if ( current_user_get_field('access_level') >= config_get( 'private_task_threshold' ) ) {
					$t_id_array[] = db_prepare_int ( $row['id'] );
			}
		} 
		//se a tarefa é pública
		else {
					$t_id_array[] = db_prepare_int ( $row['id'] );
		}
	}
		...
		$t_from = 'FROM ' . $t_bug_table . ", " . $t_project_table; //adicionando o project_table para poder filtar também pelo nome do projeto e ordenar pelo nome no relatório parcial PROSEGUR 09/06/2011
		...
		//incluído para retornar os dados ordenados pelo nome do projeto, facilita na vizualização da página PROSEGUR 12/07/2011
		$t_sort_task = split( ',', $t_filter['sort_task'] );
		if ( in_array( 'project_name', $t_sort_task ) ) {
			$t_order = 'name ' . "$t_sort_task[1], ";
        }

		$t_order = " ORDER BY " . $t_order . implode( ', ', $t_order_array );  //adicionado o $t_order que será usado quando for o relatório parcial para ordenar pelo nome do projeto
		$t_select	= implode( ', ', array_unique( $t_select_clauses ) );

		//adicionando a tabela de projeto, coluna name para poder ordenar pelo nome do projeto no relatório parcial PROSEGUR 09/06/2011
