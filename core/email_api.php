		...
		foreach ( $t_recipients as $t_id => $t_ignore ) {
			...
			if ( $t_pref_field ) {
				...
				if ( OFF == $t_notify ) {
					...
					// comentado pois o campo severidade não existe mais - PROSEGUR 28/02/2011
...
#		lang_pop(); # see above
	...
	function email_format_bug_message( $p_visible_bug_data ) {
		...
		//formato de data sem as horas PROSEGUR 20/05/2011
		...
		//formatando as datas personalizadas que foram criadas PROSEGUR 20/05/2011
		$p_visible_bug_data['start_date'] = date( $t_short_date_format, $p_visible_bug_data['start_date'] );
		$p_visible_bug_data['end_date']   = date( $t_short_date_format, $p_visible_bug_data['end_date'] );		
...
//foram inseridos campos personalizados no e-mail PROSEGUR 20/05/2011
...
//		$t_message .= email_format_attribute( $p_visible_bug_data, 'email_category' );
		...
		$t_message .= email_format_attribute( $p_visible_bug_data, 'custom_field_type' );
		$t_message .= email_format_attribute( $p_visible_bug_data, 'start_date' );
		$t_message .= email_format_attribute( $p_visible_bug_data, 'end_date' );		
		$t_message .= email_format_attribute( $p_visible_bug_data, 'hora_previsao' );						
		$t_message .= email_format_attribute( $p_visible_bug_data, 'coverage' );		
	...
	function email_build_visible_bug_data( $p_user_id, $p_bug_id, $p_message_id ) {
		...
		//campos no e-mail que foram inseridos no cadastro de uma tarefa PROSEGUR 13/03/2011
		$t_bug_data['custom_field_type'] =  get_enum_element('type', $row['type_task']);
		$t_bug_data['start_date'] = $row['date_start'];
		$t_bug_data['end_date'] = $row['date_end'];
		$t_bug_data['hora_previsao'] = $row['hours_prev'] . ' ' . lang_get('horas');	
		$t_bug_data['coverage'] = $row['coverage'] . '%';	
