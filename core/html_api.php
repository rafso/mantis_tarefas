...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	//função inserida para evitar que o título apareça na tela de login PROSEGUR 23-06-2010
	function html_page_top2a_login() {
		global $g_error_send_page_header;
		...
		$g_error_send_page_header = false;
		html_header();
		html_top_banner('login');  //alterado para diferenciar quando é chamado 
	}
	...
	function html_begin() {
		...
		echo '<html>', "\n";
		#Inserido para o loading das telas PROSEGUR 27/05/2011, problema conhecido de ser exibido no cabeçalho em alguns momentos, coloquei no estilo o dysplay none, mas mesmo assim o <br> aparece em alguns casos, verificar outra hora pois é um problema de menor gravidade
		echo '<div id="carregando" class="loading"><img id ="imgLoading" src="images/carregando.gif"><label id="txtLoading">'. lang_get("loading") .'</label></div>';	
	}
	...
	function html_head_javascript() {
		if ( ON == config_get( 'use_javascript' ) ) {
			...
			echo '</script>' . "\n";
			//inserindo o prototype usado no carregando PROSEGUR 27/05/2011
	...
	function html_top_banner($send = null) {
		...
		if ($send != 'login'){ //PROSEGUR 10-06-2010
			echo '<div class="titulo_telageral">';
					echo config_get('titulo'); 
			echo '</div>'; 
		}
			

			echo '<div align="', $t_align, '">';
			echo '<img border="0"  alt="Sistema de Gerenciamento de Atividades da FSW Prosegur" src="images/mantis_logo.gif" />';
			echo '</div>'; //PROSEGUR removido o link da logo por não fazer muito sentido 10-06-2010
		}
	}
	...
	function html_footer( $p_file ) {
		...
		if ( ON == config_get( 'show_version' ) ) {
			echo "\t", '<span class="timer"><a href="http://www.prosegur.com/" title="Sistema de Gerenciamento de Atividades da FSW Prosegur">Versao ', MANTIS_VERSION, '</a>',
					'[<a href="http://www.prosegur.com/"  title="Sistema de Gerenciamento de Atividades da FSW Prosegur" target="_blank">^</a>]</span>', "\n";
		}
		echo "\t", '<address>Copyright &copy; 2011 FSW Prosegur</address>', "\n";
		...
		echo '<a href="/tarefas" title="Prosegur FSW controle de tarefas"><img src="images/mantis_logo_button.gif" alt="FSW Prosegur" border="0" /></a>';
	...
	function print_menu() {
		if ( auth_is_user_authenticated() ) {
			...
			PRINT '<td class="menu">';
				...
				# Main Page removido a pagina principal PROSEGUR 01/06/2011
				...
				#view pool menu, ver o menu fila PROSEGUR 14/07/2011
				if ( access_has_project_level( lider ) ) {
					$t_menu_options[] = '<a href="task_pool_page.php">' . lang_get( 'pool_menu' ) . '</a>';
				}
				
				#view pool menu, ver o menu fila PROSEGUR 14/07/2011
				/* desabilitado provisoriamente pois não está pronta a página
				if ( access_has_project_level( lider ) ) {
					$t_menu_options[] = '<a href="reservation_request_page.php">' . lang_get( 'reservation_request' ) . '</a>';
				}
				*/

				# Report Bugs
				if ( access_has_project_level( config_get( 'report_bug_threshold' ) ) ) {
					//alterado para a página inicial de abertura de bugs seja a integrada
					$t_menu_options[] = '<a href="task_report_integrated_page.php">' . lang_get( 'report_bug_link' ) . '</a>';
				...
				# News Page removido a pagina de notícias PROSEGUR 01/06/2011
			...
			# Add custom options 
				...
				/* removido a pagina FATURA do menu PROSEGUR 01/06/2011
			...
			PRINT '</td>';
			PRINT '<td class="menu right nowrap" style="white-space: nowrap;">'; //alterado a classe PROSEGUR 23-06-2010
...
#				break;
	...
	function print_summary_menu( $p_page='' ) {
		...
		if ( config_get( 'use_jpgraph' ) != 0 ) {
			...
			//adicionando o relatório parcial PROSEGUR 02/06/2011
			//$t_summary_parcial_page 		= 'task_partial_report.php';
			
			switch ( $p_page ) {
				case $t_summary_page		: $t_summary_page			= ''; break;
				case $t_summary_jpgraph_page: $t_summary_jpgraph_page	= ''; break;
				//case $t_summary_parcial_page: $t_summary_parcial_page	= 'task_partial_report.php'; break;
			}
			...
			//print_bracket_link( $t_summary_parcial_page, lang_get( 'partial_report_menu' ) );
		}
		PRINT '</div>';
	}
	...
	function html_status_legend($rows = null) { //alterado para receber as linhas, para fazer o calculo das % somente dos exibidos na tela, incluindo a paginação, e não do total como era padrão PROSEGUR 02/06/2011
		PRINT '<br />';
		...
		if ( ON == config_get( 'status_percentage_legend' ) ) {
			//verifica se rows foi passada, se foi passa a diante, caso contrario segue o fluxo normal
		if (isset($rows)){
			html_status_percentage_legend($rows);
		} else {
	...
	function html_status_percentage_legend($rows = null) { //alterado para receber as linhas, para fazer o calculo das % somente dos exibidos na tela, incluindo a paginação, e não do total como era padrão PROSEGUR 02/06/2011
		...
		//faz a separação, rows enviada ou não, se foi enviada só calcula dos que estão exibidos,  incluindo a paginação
		if (isset($rows)){
		
		//query conta somente a quantidade em cada status dos ids passados
		$query = "SELECT status, COUNT(*) AS number
				FROM $t_mantis_bug_table
				WHERE $t_mantis_bug_table.id in (" . implode( ", ", $rows ) . ")
				GROUP BY status";
		...
		if ( $t_bug_count > 0 ) {
			...
			echo '<tr>';
			echo '<td class="form-title" colspan="'.$enum_count.'">'.lang_get( 'issue_status_percentage_prosegur' ), $t_bug_count .' '. lang_get( 'task_label' ).'</td>';
			echo '</tr>';
			echo '<tr>';
	...
	function html_button_bug_change_status( $p_bug_id ) {
		...
		if ( count( $t_enum_list ) > 0 ) {
			...
			//verificar se o status atual é o status que o bug se torna somente leitura e se for, remove o select de mudar o status PROSEGUR 06/03/2011
		if ($t_bug_current_state == config_get ( 'bug_readonly_status_threshold') ) {
		} else {
