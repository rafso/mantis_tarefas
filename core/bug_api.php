...
//		var $profile_id;
		...
		#variáveis da tarefa PROSEGUR 27/02/2011
		var $type_task = 0;
		var $date_start = '';
		var $date_end = '';
		var $hours_prev = 0;
		var $relation_cases = '';
		var $coverage = 0; //andamento dos testes
	}
	...
	function bug_cache_row( $p_bug_id, $p_trigger_errors=true ) {
		...
		$t_task_table	= config_get( 'mantis_task_schedule_table' );
	...
	function bug_create( $p_bug_data ) {
		...
		/* removido os campos não usados na tarefa PROSEGUR 27/02/2011
		...
		$c_sticky 				= 0;
		//inserindo os novos campos PROSEGUR 27/02/2011
		$c_type_task			= db_prepare_int( $p_bug_data->type_task );
		$c_date_start			= db_prepare_string( $p_bug_data->date_start );
		$c_date_end				= db_prepare_string( $p_bug_data->date_end );
		$c_hours_prev			= db_prepare_int( $p_bug_data->hours_prev );
		$c_relation_cases		= db_prepare_string( $p_bug_data->relation_cases );
		$c_coverage				= db_prepare_string( $p_bug_data->coverage );
		...
		$t_task_schedule_table		= config_get( 'mantis_task_schedule_table' );
		...
		$query = "INSERT INTO $t_bug_table
				  ...
				  VALUES
				    ( '$c_project_id',
				      '$c_reporter_id', '$c_handler_id', 0, 
				      '$c_priority',
				      '$t_status', '$t_resolution',
			" . db_now() . "," . db_now() . ",
			10, '$t_text_id',
			'$c_summary', '$c_view_state', '$c_type_task', '$c_relation_cases',
			CONVERT(DATETIME, '$c_date_start', 103),
			CONVERT(DATETIME, '$c_date_end', 103),
					'',
					$c_hours_prev, 0, 0
				    )";
		db_query( $query );
...
//		bug_set_field( $t_new_bug_id, 'fixed_in_version', $t_bug_data->fixed_in_version );
		...
		bug_set_field( $t_new_bug_id, 'relation_cases', $t_bug_data->relation_cases );
	...
	//função que atualiza a coluna report_submitted da tarefa, que significa que ela está com 100% de cobertura e já foi enviada pelo menos 1 vez no relatório
	//essa função é chamada no task_util após enviar o e-mail do relatório
	//recebe um array com os ids, notas e cobertura, notas são enviadas apenas por convenção, visto que esse array já está pronto e para não precisar refazer o array
	function task_update_report_submitted($rows_id) {
	$t_bug_table = config_get( 'mantis_bug_table' );
	if (is_array($rows_id)) {  //recebe array de bugs
	...
	function bug_update( $p_bug_id, $p_bug_data, $p_update_extended = false, $p_bypass_mail = false ) {
		...
		//recebe os dados para salvar no banco PROSEGUR 05/03/2011
		$c_bug_data->hours_prev		= db_prepare_int( $p_bug_data->hours_prev );
		$c_bug_data->date_start		= $p_bug_data->date_start;
		$c_bug_data->date_end		= $p_bug_data->date_end;
		$c_bug_data->type_task		= $p_bug_data->type_task;
		$c_bug_data->coverage		= $p_bug_data->coverage;
	...
	$query = "UPDATE $t_bug_table
			...
			hours_prev=  '$c_bug_data->hours_prev',
			type_task= '$c_bug_data->type_task',
			coverage= '$c_bug_data->coverage',";
		
		//verificar se o formato da data é normal ou unix time stamp, dependendo da pagina chamadora pode haver essa diferença, atualizar o caso envia como data normal, atualizar o status envia como unix timestamp PROSEGUR 06/03/2011
	if (strpbrk($c_bug_data->date_start, '/-'))	{
		$query .= "date_start= CONVERT(DATETIME, '$c_bug_data->date_start', 103),
				   date_end= CONVERT(DATETIME, '$c_bug_data->date_end', 103),";
	} else {
		$start_time_convert = date('Y-m-d H:i:s', $c_bug_data->date_start); 
		$end_time_convert = date('Y-m-d H:i:s', $c_bug_data->date_end); 
		$query .= "date_start= '$start_time_convert',
				date_end= '$end_time_convert',";
	}
	
	//PROSEGUR - 13/05/2011 - verificar se está alterando para o ultimo status (100), se estiver, seta o data_finished para a data e hora atual...
	//existe um "problema" conhecido, que ao atualizar uma tarefa estando no status 100, será alterado também a data_finished, porém essa ação de atualizar um caso
	//depois de fechado não é comum e somente o usuário administrador possui a permissão.
	if ($c_bug_data->status == '100') {
		$date_fin = date('Y-m-d H:i:s');
		$query .= "date_finished = '$date_fin',";
	}
...
//		history_log_event_direct( $p_bug_id, 'fixed_in_version', $t_old_data->fixed_in_version, $p_bug_data->fixed_in_version );
		...
		//adicionando as marcações no histórico para os campos adicionados na tarefa PROSEGUR 24/05/2011
	//verificar se o formato da data é normal ou unix time stamp, dependendo da pagina chamadora pode haver essa diferença, atualizar o caso envia como data normal, atualizar o status envia como unix timestamp PROSEGUR 06/03/2011
	if (strpbrk($p_bug_data->date_start, '/-'))	{
		history_log_event_direct( $p_bug_id, lang_get('start_date'), date( config_get( 'short_date_format' ),$t_old_data->date_start), $p_bug_data->date_start);
		history_log_event_direct( $p_bug_id, lang_get('end_date'), date( config_get( 'short_date_format' ),$t_old_data->date_end), $p_bug_data->date_end);
	} else {
		history_log_event_direct( $p_bug_id, lang_get('start_date'), date( config_get( 'short_date_format' ),$t_old_data->date_start), date( config_get( 'short_date_format' ),$p_bug_data->date_start) );
		history_log_event_direct( $p_bug_id, lang_get('end_date'), date( config_get( 'short_date_format' ),$t_old_data->date_end), date( config_get( 'short_date_format' ),$p_bug_data->date_end) );
	}

		history_log_event_direct( $p_bug_id, 'type', $t_old_data->type_task, $p_bug_data->type_task );
		history_log_event_direct( $p_bug_id, lang_get('coverage'), $t_old_data->coverage, $p_bug_data->coverage );		
		history_log_event_direct( $p_bug_id, lang_get('history_update_hours_prev'), $t_old_data->hours_prev, $p_bug_data->hours_prev );		
...
//função que atualiza a cobertura de uma tarefa, grava no histórico e envia um e-mail informando da mudança PROSEGUR 20/05/2011
function task_coverage_update( $task_id, $new_value) {
	$c_task_id		= db_prepare_int( $task_id );
	$t_old_data = bug_get( $c_task_id, true );
	...
	# log changes
	history_log_event_direct( $task_id, lang_get('coverage'), $t_old_data->coverage, $new_value );
	
	//verifica se a atividade estava com andamento 0 e foi atualizado, nesse caso muda o status para em andamento automaticamente sem perguntar nada ao usuário PROSEGUR 31/08/2011
	if (($t_old_data->coverage == 0) & (bug_get_field($task_id, 'status') < 50)) {
		bug_set_field($task_id, 'status', 50);
		# envia o e-mail
		email_generic( $task_id, 'updated', $t_action_prefix . 'email_notification_title_for_status_bug_doing' );
	}
	
	# Update the last update date
	bug_update_date( $task_id );
	
	#verifica se a cobertura é >= ao definido em main_menu_custom_options no config_inc para enviar e-mail
	if (config_get('min_value_send_mail_coverage') <= $new_value) {
		# envia o e-mail
...
//função que atualiza a hora prevista de uma tarefa, grava no histórico e envia um e-mail informando da mudança PROSEGUR 24/05/2011
function task_hours_prev_update( $task_id, $new_value) {
	$c_task_id		= db_prepare_int( $task_id );
	$t_old_data = bug_get( $c_task_id, true );
	...
	# log changes
	history_log_event_direct( $task_id, lang_get('history_update_hours_prev'), $t_old_data->hours_prev, $new_value );
	
	# Update the last update date
	bug_update_date( $task_id );
	
	# envia o e-mail
	email_generic( $task_id, 'updated', $t_action_prefix . 'email_notification_title_for_update_hours_prev' );
	...
	function bug_resolve( $p_bug_id, $p_resolution, $p_bugnote_text = '', $p_duplicate_id = null, $p_handler_id = null, $p_bugnote_private = false, $p_time_tracking = '0:00' ) {
		$p_bugnote_text = trim( $p_bugnote_text );
