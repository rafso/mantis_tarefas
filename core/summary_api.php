...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	$t_resolved_val = waitbugs; //alterado de RESOLVED para waitbugs PROSEGUR 02/06/2011
		$t_closed_val = closed; //alterado de CLOSED para closed PROSEGUR 01/06/2011
			...
			if ( ( $row[$p_enum] != $t_last_value ) &&
				...
				switch ( $p_enum ) {
					case 'status':
						...
						break;
					case 'type_task':
						$t_bug_link = '<a class="subtle" href="' . $t_filter_prefix . '&amp;show_type=' . $t_last_value;
						break;
					case 'resolution':
				...
				if ( !is_blank( $t_bug_link ) ) {
					if ( 0 < $t_bugs_open ) {
					$t_bugs_open = $t_bug_link . '&amp;hide_status=' . closed . '">' . $t_bugs_open . '</a>'; //alterado de RESOLVED para closed PROSEGUR 01/06/2011
					} else {
						if ( ( 'status' == $p_enum ) && ( $t_last_value >= $t_resolved_val ) ) {
							$t_bugs_open = '-';
						}
					}
					//adicionado validação extra, verificando se é igual a waitbugs, porque estava colocando no filtro 2x a opção show_status
					if (( 0 < $t_bugs_resolved ) && ($t_last_value == waitbugs) ){
					$t_bugs_resolved = $t_bug_link . '&amp;hide_status=' . closed . '">' . $t_bugs_resolved . '</a>'; //alterado de CLOSED para closed e de RESOLVED para waitbugs PROSEGUR 01/06/2011
					}
					if (( 0 < $t_bugs_resolved ) && ($t_last_value != waitbugs) ){
					$t_bugs_resolved = $t_bug_link . '&amp;show_status=' . waitbugs . '&amp;hide_status=' . closed . '">' . $t_bugs_resolved . '</a>'; //alterado de CLOSED para closed e de RESOLVED para waitbugs PROSEGUR 01/06/2011
					} else {
					...
					if ( 0 < $t_bugs_closed ) {
					$t_bugs_closed = $t_bug_link . '&amp;show_status=' . closed . '&amp;hide_status=">' . $t_bugs_closed . '</a>'; //alterado de CLOSED para closed PROSEGUR 01/06/2011
					} else {
	...
	function summary_print_by_project( $p_projects = null, $p_level = 0, $p_cache = null ) {
		...
		$t_resolved_val = waitbugs; //alterado de RESOLVED para closed waitbugs 01/06/2011
		$t_closed_val = closed; //alterado de CLOSED para closed PROSEGUR 01/06/2011
			...
			while ( $row = db_fetch_array( $result ) ) {
				...
				if ( isset( $p_cache[ $v_project_id ][ 'waitbugs' ] ) ) {
					$p_cache[ $v_project_id ][ 'waitbugs' ]  += $v_bugcount; //alterado de RESOLVED para closed waitbugs 01/06/2011
					} else {
					$p_cache[ $v_project_id ][ 'waitbugs' ]  = $v_bugcount; //alterado de RESOLVED para closed waitbugs 01/06/2011
					}
				} else {
		...
		$t_bugs_resolved = isset( $t_pdata['waitbugs'] ) ? $t_pdata['waitbugs'] : 0; //alterado de RESOLVED para closed waitbugs 01/06/2011
