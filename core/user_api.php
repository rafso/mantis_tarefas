...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------
	# retorna todos os usuários com um nível de acesso >= que o informado
	function user_get_all_level( $p_level=ANYBODY ) {
		$t_level = db_prepare_int( $p_level );
		$t_user_table = config_get( 'mantis_user_table' );
		$query = "SELECT id, username, realname FROM $t_user_table WHERE access_level>=$t_level";
		$result = db_query( $query );
	...
	# --------------------
	# retorna todos os usuários com um nível de acesso = que o informado
	function user_get_with_level( $p_level=ANYBODY ) {
		$t_level = db_prepare_int( $p_level );
		$t_user_table = config_get( 'mantis_user_table' );
		$query = "SELECT id, username, realname FROM $t_user_table WHERE access_level=$t_level";
		$result = db_query( $query );
	...
	# retorna todos os usuários com um nível de acesso = que o informado no projeto informado/selecionado
	//se for informado um projeto traz somente os usuários com o level requisitado daquele projeto, se não tras todos os usuário com o level informado global
	function user_project_get_with_level( $p_level=ANYBODY, $p_project = ALL_PROJECTS ) {
		$t_level = db_prepare_int( $p_level );
		...
		//projeto informado
		if ($t_project > 0){
			$user_level = project_get_local_user_access_level( $t_project, $user['id']); //pega o perfil do usuário no projeto selecionado
			if ($user_level == $t_level){  //verifica se o perfil do usuário no projeto é o perfil requisitado
				$t_users[] = $user;
			}
...
*/
	...
	*/
	//foi alterado para funcionar com a imagem local, pega o username e procura, os dados são configurados no config.php PROSEGUR 01/06/2011	
	function user_get_avatar( $p_user_id ) {
		$t_username = strtolower( user_get_field( $p_user_id, 'username' ) );
		if ( is_blank( $t_username ) ) {
			$t_result = false;
		} else {
			$t_avatar_url = config_get('folder_avatar') . $t_username . "*";
			//usa a função glob para pesquisar pelo arquivo nomeuser.* no diretório PROSEGUR 07/06/2011
			$file = glob($t_avatar_url);
			$t_avatar_url = $file[0];
			//verifica se existe um avatar, se não existir enviar o padrão PROSEGUR 02/06/2011
			if ( file_exists($t_avatar_url) ) { //verifica se existe um avatar do usuário, se não existir exibe o padrão
				$t_result = array( $t_avatar_url);
			} else {
				$t_default_image = config_get( 'default_avatar' );
				$t_result = array( $t_default_image);
			}
		}
